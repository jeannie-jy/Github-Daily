...
async function getLiveDigest() {
  ...
  const search = date => github(`/search/repositories?q=${encodeURIComponent(`created:>${new Date(date - 7 * 864e5).toISOString().slice(0, 10)}`)}&sort=stars&order=desc&per_page=12`);
  let results;
  ...
  const trending = results.items.map((repo, index) => ({ name: repo.full_name, icon: icon(repo.language), score: `Top ${index + 1}`, description: repo.description || '本周新出现的热门开源项目。', stars: compact(repo.stargazers_count), language: repo.language || 'Open source', delta: '新成果' }));
  const releases = (await Promise.all(trending.slice(0, 8).map(async repo => { try { const release = await github(`/repos/${repo.name}/releases/latest`); return { name: repo.name, icon: repo.icon, detail: `${release.tag_name} · ${release.name || '最新版本'}`, publishedAt: release.published_at }; } catch { return null; } }))).filter(Boolean).slice(0, 5);
